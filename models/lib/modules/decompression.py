...
class idct_8x8(nn.Module):
    ...
    def forward(self, image):
        if image.is_cuda:
            ...
            self.alpha = self.alpha.cuda(image.get_device())
...
class block_merging(nn.Module):
    ...
    def forward(self, patches, height, width):
        ...
        hp = 8-height%8 if height%8 != 0 else 0
        wp = 8-width%8 if width%8 != 0 else 0

        img_unf = patches.view(batch_size, -1, 64)
        img_unf = img_unf.permute(0, 2, 1)
        img_out = F.fold(img_unf, output_size=(height+hp, width+wp), kernel_size=(k, k),
                         stride=(k, k))

        img_out = img_out.squeeze(1)
        return img_out.contiguous()
    ...
    def forward(self, y, cb, cr, height, width):
        ...
        for k in components.keys():
            if k in ('cb', 'cr'):
                ...
                height_, width_ = math.ceil(height/2), math.ceil(width/2)                
            else:
                comp = self.y_dequantize(components[k])
                height_, width_ = height, width                
            comp = self.idct(comp)
            components[k] = self.merging(comp, height_, width_)

        image = self.chroma(components['y'], components['cb'], components['cr'], height, width)
